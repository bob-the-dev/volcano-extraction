...
## Simple HUD controls for raising and lowering the procedural water level.
...
func _update_label() -> void:
	if _procedural_map == null:
		_value_label.text = "Water unavailable"
		return

	var current_level: float = 0.0
	var target_level: float = 0.0
	if _procedural_map.has_method("get_water_height_level"):
		current_level = float(_procedural_map.call("get_water_height_level"))
	if _procedural_map.has_method("get_target_water_height_level"):
		target_level = float(_procedural_map.call("get_target_water_height_level"))
	else:
		target_level = current_level

	_value_label.text = "Water %.2f -> %.2f" % [current_level, target_level]
...
func _on_raise_pressed() -> void:
	...
	_procedural_map.call("raise_water_height")
	_update_label()
...
func _on_lower_pressed() -> void:
	...
	_procedural_map.call("lower_water_height")
	_update_label()
